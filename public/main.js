...
const treatNotification = document.getElementById('treatNotification');
...
let isDogMode = true;
let humanGroup = null;
let wardrobeGroup = null;
let dogTreatMesh = null;
let treatCollected = false;
let heartEffectActive = false;
const WARDROBE_SWITCH_DIST = 4.0;
const WARDROBE_TREAT_DIST = 3.0;
...
// 바닥 격자 선 (도시 보도블록 느낌)
const gridHelper = new THREE.GridHelper(100, 40, 0x2a2a2a, 0x1c1c1c);
...
/* ───────────────────────────────
   ...
   ─────────────────────────────── */
// 문(스폰 지점) 변수 정의
const doorPosition = new THREE.Vector3(-8, 0, 8);

function createIndoorFurniture() {
  const woodMat = new THREE.MeshStandardMaterial({ color: 0x4a3525, roughness: 0.8, metalness: 0.1 });
  
  // 책상 (Desk)
  const deskGroup = new THREE.Group();
  const deskTop = new THREE.Mesh(new THREE.BoxGeometry(3, 0.2, 1.5), woodMat);
  ...
  deskGroup.position.set(-4, 0, -5);
  scene.add(deskGroup);

  // 화병 (Vase)
  const vaseGeo = new THREE.CylinderGeometry(0.15, 0.1, 0.4, 12);
  const vaseMat = new THREE.MeshStandardMaterial({ color: 0x88aacc, roughness: 0.3, metalness: 0.2 });
  const vase = new THREE.Mesh(vaseGeo, vaseMat);
  vase.position.set(-1, 1.8, 0);
  ...
  const backrest = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 0.1), woodMat);
  ...
  cLegPositions.forEach(pos => {
    const leg = new THREE.Mesh(cLegGeo, woodMat);
    leg.position.set(...pos);
    leg.castShadow = true;
    chairGroup.add(leg);
  });
...
// 옷장 본체
const wardrobeMat = new THREE.MeshStandardMaterial({ color: 0x3a2515, roughness: 0.8, metalness: 0.1 });
const wardrobeBody = new THREE.Mesh(new THREE.BoxGeometry(2.4, 4.0, 1.4), wardrobeMat);
wardrobeBody.position.y = 2.0;
wardrobeBody.castShadow = true;
wardrobeGroup.add(wardrobeBody);

// 옷장 문 라인 (왼쪽/오른쪽)
const doorLineMat = new THREE.MeshStandardMaterial({ color: 0x2a1a0a, roughness: 0.7 });
const doorLineGeo = new THREE.BoxGeometry(0.04, 3.6, 0.02);
const doorLineCenter = new THREE.Mesh(doorLineGeo, doorLineMat);
doorLineCenter.position.set(0, 2.0, 0.71);
wardrobeGroup.add(doorLineCenter);

// 문 손잡이
const knobGeo = new THREE.SphereGeometry(0.06, 8, 8);
const knobMat = new THREE.MeshStandardMaterial({ color: 0xccaa66, metalness: 0.6, roughness: 0.3 });
const knobL = new THREE.Mesh(knobGeo, knobMat);
knobL.position.set(-0.15, 2.0, 0.72);
wardrobeGroup.add(knobL);
const knobR = new THREE.Mesh(knobGeo, knobMat);
knobR.position.set(0.15, 2.0, 0.72);
wardrobeGroup.add(knobR);

wardrobeGroup.position.set(5, 0, -10);
scene.add(wardrobeGroup);

// 개껌 (옷장 위에 놓임)
const treatGeo = new THREE.CylinderGeometry(0.12, 0.12, 0.6, 12);
const treatMat = new THREE.MeshStandardMaterial({
  color: 0xD4A04A, roughness: 0.6, metalness: 0.1,
  emissive: 0x664400, emissiveIntensity: 0.3
});
dogTreatMesh = new THREE.Mesh(treatGeo, treatMat);
dogTreatMesh.rotation.z = Math.PI / 2;
dogTreatMesh.position.set(5, 4.3, -10);
dogTreatMesh.castShadow = true;
scene.add(dogTreatMesh);

// 개껌 글로우 링
const treatRingGeo = new THREE.TorusGeometry(0.3, 0.02, 8, 24);
const treatRingMat = new THREE.MeshStandardMaterial({
  color: 0xffaa00, emissive: 0xffaa00, emissiveIntensity: 0.8,
  transparent: true, opacity: 0.5
});
const treatRing = new THREE.Mesh(treatRingGeo, treatRingMat);
treatRing.position.copy(dogTreatMesh.position);
treatRing.rotation.x = Math.PI / 2;
scene.add(treatRing);
...
const humanBodyMat = new THREE.MeshStandardMaterial({ color: 0x445566, roughness: 0.7, metalness: 0.1 });

// 몸통
const hBodyGeo = new THREE.CapsuleGeometry(0.35, 1.2, 8, 16);
const hBody = new THREE.Mesh(hBodyGeo, humanBodyMat);
hBody.position.y = 1.5;
humanGroup.add(hBody);

// 머리
const hHeadGeo = new THREE.SphereGeometry(0.28, 12, 10);
const hHeadMat = new THREE.MeshStandardMaterial({ color: 0xE8C9A0, roughness: 0.7 });
const hHead = new THREE.Mesh(hHeadGeo, hHeadMat);
hHead.position.y = 2.45;
humanGroup.add(hHead);

// 다리
const hLegGeo = new THREE.CapsuleGeometry(0.12, 0.6, 6, 10);
const hLegMat = new THREE.MeshStandardMaterial({ color: 0x333344, roughness: 0.8 });
const hLegL = new THREE.Mesh(hLegGeo, hLegMat);
hLegL.position.set(-0.18, 0.4, 0);
humanGroup.add(hLegL);
const hLegR = new THREE.Mesh(hLegGeo, hLegMat);
hLegR.position.set(0.18, 0.4, 0);
humanGroup.add(hLegR);

humanGroup.position.set(100, 0, 100); // 화면 밖 대기
...
function createHeartTexture() {
  ...
  cx.beginPath();
  cx.moveTo(32, 56);
  cx.bezierCurveTo(4, 36, 4, 12, 32, 20);
  cx.bezierCurveTo(60, 12, 60, 36, 32, 56);
  cx.fill();
...
function spawnHeartEffect() {
  if (heartEffectActive) return;
  heartEffectActive = true;
  for (let i = 0; i < HEART_COUNT; i++) {
    const sMat = new THREE.SpriteMaterial({
      map: heartTex, transparent: true, opacity: 0.9,
  ...
  setTimeout(() => { heartEffectActive = false; }, 2500);
}

function updateHearts(delta) {
  for (let i = heartSprites.length - 1; i >= 0; i--) {
    const h = heartSprites[i];
    if (h.delay > 0) { h.delay -= delta; continue; }
    h.life -= delta;
    if (h.life <= 0) {
    ...
    h.sprite.position.x += h.vx * delta;
    h.sprite.position.y += h.vy * delta;
    h.sprite.position.z += h.vz * delta;
    h.vy -= 0.5 * delta;
    h.sprite.material.opacity = Math.min(1, h.life);
    const s = 0.2 + 0.15 * Math.sin(h.life * 6);
    h.sprite.scale.set(s, s, s);
  }
}
...
const smellGoal = new THREE.Vector3(5, 0.1, -10);
...
function handleModeSwitch() {
  if (!isDogMode) return; // 사람 모드에서는 Shift로 돌아가지 않음 (개껌 전달 시 자동 복귀)
  if (treatCollected) return; // 이미 퍼즐 완료

  // 옷장 근처인지 확인
  const wardrobePos = new THREE.Vector3();
  wardrobeGroup.getWorldPosition(wardrobePos);
  const dist = dogGroup.position.distanceTo(wardrobePos);
  if (dist > WARDROBE_SWITCH_DIST) return;

  // 강아지 → 사람 전환
  isDogMode = false;
  moveState.forward = moveState.backward = moveState.left = moveState.right = false;

  // 사람 캐릭터를 문(스폰 지점)에 스폰
  humanGroup.position.copy(doorPosition);
  humanGroup.visible = true;

  // UI 업데이트
  if (switchHint) switchHint.classList.add('hidden');
  if (modeIndicator) {
    modeIndicator.textContent = '🧑 사람 모드';
    modeIndicator.classList.add('human-mode');
  }
  console.log('[SNOOP] 사람으로 전환됨. 옷장 위의 개껌을 가져가세요!');
}

function handleTreatGrab() {
  if (isDogMode || treatCollected) return;
  if (!dogTreatMesh || !dogTreatMesh.visible) return;

  // 사람이 옷장 근처인지 확인
  const wardrobePos = new THREE.Vector3();
  wardrobeGroup.getWorldPosition(wardrobePos);
  const dist = humanGroup.position.distanceTo(wardrobePos);
  if (dist > WARDROBE_TREAT_DIST) return;

  treatCollected = true;

  // a. 개껌 사라짐
  dogTreatMesh.visible = false;
  // 글로우 링도 숨김 (treatRing은 씬에서 이름으로 찾기 어려우므로 위치로 판별)
  scene.children.forEach(child => {
    if (child.geometry && child.geometry.type === 'TorusGeometry' &&
        Math.abs(child.position.x - 5) < 0.1 && Math.abs(child.position.z + 10) < 0.1 &&
        child.position.y > 4) {
      child.visible = false;
    }
  });
  ...
  // b. 알림 표시
  console.log('[SNOOP] 개껌을 강아지에게 전달했다!');
  if (treatNotification) {
    treatNotification.classList.remove('hidden');
    setTimeout(() => { treatNotification.classList.add('hidden'); }, 2500);
  }

  // c. 하트 이펙트
  spawnHeartEffect();

  // d. 2초 후 강아지 모드로 복귀, 사람 퇴장
  setTimeout(() => {
    isDogMode = true;
    humanGroup.visible = false;
    humanGroup.position.set(100, 0, 100);
    moveState.forward = moveState.backward = moveState.left = moveState.right = false;

    if (modeIndicator) {
      modeIndicator.textContent = '🐕 강아지 모드';
      modeIndicator.classList.remove('human-mode');
    }
    console.log('[SNOOP] 강아지 모드로 복귀!');
  }, 2000);
}
...
function animate() {
  ...
  /* ─── 캐릭터 이동 (WASD) — isDogMode 분기 ─── */
  const activeGroup = isDogMode ? dogGroup : humanGroup;
  if (isPointerLocked) {
    ...
    if (moveDir.lengthSq() > 0) {
      ...
      activeGroup.position.add(moveDir);
      ...
      if (isDogMode) {
        ...
        const targetAngle = Math.atan2(moveDir.x, moveDir.z);
        humanGroup.rotation.y = targetAngle;
      }
    }
  }

  // 점프 물리 (중력 적용 → 착지 고정) — 강아지 모드에서만
  if (isDogMode) {
  ...
  /* ─── 다리 걷기 애니메이션 (강아지 모드일 때만 이동 연동) ─── */
  const isMoving = isDogMode && isPointerLocked && (
  ...
  /* ─── 3인칭 숄더뷰 카메라 (현재 조작 대상 추적) ─── */
  const camTarget = isDogMode ? dogGroup : humanGroup;
  const camHeightOffset = isDogMode ? 0.5 : 1.5;
  const idealOffset = new THREE.Vector3(
    Math.sin(yaw) * CAM_DISTANCE,
    CAM_HEIGHT,
    Math.cos(yaw) * CAM_DISTANCE
  );
  const idealPos = camTarget.position.clone().add(idealOffset);
  camera.position.lerp(idealPos, CAM_LERP);

  const lookTarget = new THREE.Vector3(
    camTarget.position.x,
    camTarget.position.y + camHeightOffset,
  ...
  if (isDogMode && !treatCollected && wardrobeGroup) {
    ...
    wardrobeGroup.getWorldPosition(wPos);
    const distToWardrobe = dogGroup.position.distanceTo(wPos);
    if (switchHint) {
      switchHint.classList.toggle('hidden', distToWardrobe > WARDROBE_SWITCH_DIST);
    }
  } else if (switchHint) {
    switchHint.classList.add('hidden');
  }

  /* ─── 사람 모드: 옷장 근처 개껌 상호작용 힌트 ─── */
  if (!isDogMode && !treatCollected && dogTreatMesh && dogTreatMesh.visible) {
    const wPos2 = new THREE.Vector3();
    wardrobeGroup.getWorldPosition(wPos2);
    const distH = humanGroup.position.distanceTo(wPos2);
    if (interactionHint) {
      interactionHint.textContent = 'E 키를 눌러 개껌 가져가기';
      interactionHint.classList.toggle('hidden', distH > WARDROBE_TREAT_DIST);
    }
  }

  /* ─── 개껌 글로우 링 회전 ─── */
  if (dogTreatMesh && dogTreatMesh.visible) {
    dogTreatMesh.rotation.y = elapsed * 1.5;
  }
